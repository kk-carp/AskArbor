...
def test_answer_question_followup_expands_retrieval_query(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    conversation_id = str(uuid4())
    seen: dict[str, str] = {}
    ...
    assert result.hit is False
    assert seen["query"] == "课程作业怎么交 那截止日期呢"


def test_answer_question_complete_follow_topic_does_not_expand_query(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    conversation_id = str(uuid4())
    seen: dict[str, str] = {}
