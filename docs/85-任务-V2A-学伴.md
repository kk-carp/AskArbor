...
> **产品入口变更：** V2-A4 原「学习路径」API/前端已删除；请用进阶资料推荐验收（见 `docs/06-进阶资料推荐.md`）。下列 A4 步骤保留为历史任务说明。

| 项    | 内容                                                         |
| ---- | ---------------------------------------------------------- |
| 对应需求 | `docs/84-需求-V2A-学伴.md`；基线见 `docs/80-需求-总览.md` 第 8.1、8.3、9–11 节 |
| 对应架构 | `docs/03-系统架构.md` 第 10.2 节                                  |
| 对应选型 | `docs/04-技术选型.md` 第 9.2 节（新依赖先改选型再改 `requirements.txt`）     |
...
6. **新依赖：** OCR 引擎、网页搜索供应商等先在本任务内拍板并写入 `docs/04-技术选型.md` 第 9.2 节，再改 `requirements.txt` / Compose。引擎/搜索 API 未拍板前不写入新依赖。A4 课外论文检索复用 `httpx` + arXiv 白名单主机。
...
**目标：** 暂不实现。基线需求仍见 `docs/80-需求-总览.md` §8.1.2。
...
**开工前：** 把课外搜索供应商与超时写入 `docs/04-技术选型.md` §9.2（默认 Tavily + arXiv，见需求 2.3 / 2.4）。对标品牌有则写入配置说明。
...
**开工前拍板：** OCR 引擎（建议 PaddleOCR）写入 `docs/04-技术选型.md` 第 9.2 节后再加依赖。
...
与 `docs/84-需求-V2A-学伴.md` 第 8 节、`docs/80-需求-总览.md` 第 11 节「V2 学伴」对齐。建议 A1+A5 后做一次，A4 完成后再补：
...
- V2-B：入职推荐（问答内）。公司 Skill 另开。见 `docs/86-需求-V2B-助手.md`。
...
- 需求细节：`docs/84-需求-V2A-学伴.md`
- 总表摘要：`docs/81-任务-MVP.md`「V2-A：学伴」
- 架构扩展点：`docs/03-系统架构.md` §10.2
- 选型：`docs/04-技术选型.md` §9.2（本阶段只用有直接用途的依赖）
- 产品入口：`docs/88-需求-前端.md`（学伴页为该前端第二期，跟 API，不阻塞 A1、A4、A5）
- 智能助手：`docs/86-需求-V2B-助手.md` / `docs/87-任务-V2B-助手.md`
- V1 任务史：`docs/83-任务-V1.md`
